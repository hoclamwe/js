...
if(window.location.hostname=="minhngoc.tv")
window.location="http://www.xosohomnay.com";/*
...
function setActive(e) {
	$(e).children("li a").each(function (index, element) {
...
function getajaxcontent(e, t, n) {
	$("#" + e).html('<center><br>Đang xử lý...<center>');
	$.ajax({
		type: "GET",
		url: t,
		data: n,
...
function getLotoDonVi(e) {
	$("#DDM" + e).html('<div id="DDMfull' + e + '"><center><br>Đang xử lý...<center></div>');
	$.ajax({
		type: "GET",
...
function print_vedo(mien, date) {
	if($(".dailog_bog").length==0) $("body").append('<div class="dailog_bog"></div>');
	var am = {
		1: "mien-nam",
		2: "mien-bac",
		3: "mien-trung",
		115: "dien-toan-mega-645",
		116: "dien-toan-max4d",
		117: "dien-toan-power-655"
	};
	var mienurl = am[mien];
	$(".dailog_bog").dialog({
		modal: true,
		dialogClass: 'boxchonloaivedo',
		open: function () {
			if (date != "")
			...
			$(".boxselectforprintvedo a.print").click(function (e) {
...
if (window.location.href.indexOf("#ngay-") != -1) {
	$("head").append('<style>#page_content{ opacity: 0; padding-top:80px}</style>');
}

function KQXSOnload(url) {
	var query = null;
	if (url.indexOf("#ngay-") != -1) {
		var ngay = url.split("#ngay-")[1];

		if (ngay.length == 10) {
			$("#pagecontent").prepend('<div class="ajaxload"><center><br>Đang xử lý...<center></div>');
	...
	if (query != null) {
		...
		$.ajax({
			...
			.done(function (msg) {
				...
				$("#page_content").css("padding-top", "80px");
...
function changecssclass() {
	...
	if (tivimod == 1) {
		...
		$("#fulltt").attr("class", "fullttH");
		var tylewh = window.innerHeight / window.innerWidth;
		$("#fulltt").width(1280 * tylewh);
		var zw = window.innerHeight / $("#fulltt").width();
		var zh = window.innerWidth / $("#fulltt").height();
		var zoom;
		if (zw > zh) zoom = zh;
		else
			zoom = zw;

		$("#fulltt").css("margin-top", (window.innerWidth - $("#fulltt").height()) / 2);
		$("#fulltt").css("margin-left", (window.innerHeight - $("#fulltt").width()) / 2);


		$("#fulltt").css("-moz-transform", "scale(" + zoom + ")");
		$("#fulltt").css("-webkit-transform", "scale(" + zoom + ")");
		$("#fulltt").css("-o-transform", "scale(" + zoom + ")");
		...
		if (window.innerHeight > window.innerWidth) {
			...
			if (window.innerHeight / window.innerWidth < 1.4) {
				$("#fulltt").attr("class", "fullttH43");
				var tylewh = window.innerWidth / window.innerHeight;
				$("#fulltt").width(1024 * tylewh);
			} else {
				$("#fulltt").attr("class", "fullttH");
				var tylewh = window.innerWidth / window.innerHeight;
				$("#fulltt").width(1280 * tylewh);
			...
			if (window.innerWidth / window.innerHeight < 1.5) {
				$("#fulltt").attr("class", "fullttW43");
				var tylewh = window.innerWidth / window.innerHeight;
				$("#fulltt").width(768 * tylewh);

			} else {
				$("#fulltt").attr("class", "fullttW");
				var tylewh = window.innerWidth / window.innerHeight;
				$("#fulltt").width(720 * tylewh);
		...
		var zw = window.innerWidth / $("#fulltt").width();
		var zh = window.innerHeight / $("#fulltt").height();
		var zoom;
		if (zw > zh) zoom = zh;
		else
			zoom = zw;
		$("#fulltt").css("top", (window.innerHeight - $("#fulltt").height()) / 2 + "px");
		$("#fulltt").css("left", (window.innerWidth - $("#fulltt").width()) / 2 + "px");
...
function setToolBkqxs() {
	...
	$(".btnlinkprint").click(function (e) {
		...
		var mien = idp.split("-")[0];
		var ngay = right(idp, 10);
		print_vedo(mien, ngay);
		return false;
	});
	...
	$(".btnshare").click(function (e) {
		if ($(this).hasClass('clicked')) {
			...
			$(".boxsharekqxs .fn-share-zing").click(function () {
				var a = "http:/"+"/link.apps.zing.vn/share?u=" + siteDomain + "#ngay-" + date + "&t=" + title[id] + ' ngày ' + date + "";
				window.open(a, "share", "height=400,width=600");
				return false
			});
			$(".boxsharekqxs .fn-share-fb").click(function () {
				var a = 'https:/'+'/www.facebook.com/sharer/sharer.php?u=' + siteDomain + url[id] + "#ngay-" + date;
				window.open(a, "share", "height=400,width=600");
				return false
			});
			$(".boxsharekqxs .fn-share-gplus").click(function () {
				var a = "https:/"+"/plus.google.com/share?url=" + siteDomain + url[id] + "#ngay-" + date + "&t=" + title[id] + ' ngày ' + date + '';
				window.open(a, "share", "height=400,width=600");
				return false
			})
	...
	$(function () {
		$('.btnfullsize').click(function () {
			...
			$('body').prepend('<div id="fullscreen"><div id="fulltt" class="fullttH"><button class="miximize_icon"></button></div></div>');
			$("body").css({
				overflow: 'hidden'
			});
	...
	$(".btntk2sc").click(function (e) {
		...
		if ($(this).hasClass("btnclicked")) {
			...
			$("#DDM" + t).html('<div id="DDMfull' + t + '"></div>');
			$.ajax({
				type: "GET",
	...
	$("#navdateketqua a").click(function (e) {
...
function xoay90() {
	...
	$("body").append('<iframe src="/toll/rotate_report.html" width="100%" height="100%" style="display:none" frameborder="0"></iframe>');
}
function settoolloto() {
						...
						$( ".btnlotohandonvi" ).click( function ( e ) {
							$( this ).toggleClass( "off" );
							$( "#b_ddtt_loto" ).toggle( 300 );
							$( ".b_dd" ).each( function ( index, element ) {
								$( this ).show();
							} );
...
function checklockfull() {
	if ( getCookie( 'fulltructiep' ) == 1 ) {
		$( ".LockFullicon" ).addClass( "locked" );
	} else {
		$( ".LockFullicon" ).removeClass( "locked" );
	}
}
...
function setfulltt() {
	...
	$( 'body' ).prepend( '<div id="fullscreen"><div id="fulltt" class="fullttH"><button class="miximizeicon"></button></div><div class="fullscreenmenu"><a href="/truc-tiep-xo-so-mien-nam.html">Miền Nam</a><a href="/truc-tiep-xo-so-mien-trung.html">Miền Trung</a><a href="/truc-tiep-xo-so-mien-bac.html">Miền Bắc</a><button class="LockFullicon" title="Bật tắt chế độ luôn phóng to toàn màn hình"></button></div></div>' );
	$( "body" ).css( {
		overflow: 'hidden'
	} );
	if ( tivimod != 1 )
	...
	$.ajax( {
		...
		success: function ( t ) {
			...
			var element = $( '#ngheradio' ).detach();
			$( '#fullscreen' ).append( element );
			$( '#ngheradio' ).addClass( 'fullscreenradio' );
...
function startpage_xstt() {
	...
	setInterval( function () {
		if ( returntime > 0 ) returntime = returntime + 1;
	}, 1000 );

	setInterval( function () {
		strUrl = strUrlmain;
		console.log( "Check Root URL " + strUrl );
	}, 120000 );
...
function startpage_xsttdai(){
	ion.sound({
            sounds: [
                {name: "beep"},{name: "error"}
            ],
            path: "/jquery/",
            preload: true,
            volume: 1.0
			...
			setInterval(function(){ if(returntime>0)returntime=returntime+1; },1000);
			
			setInterval(function() {strUrl=strUrlmain;console.log("Check Root URL "+strUrl);},120000);
}
...
function navkqxsdate()
{
$(function() {
					$( "#getngaykqxs_1,#getngaykqxs_2").datepicker({
						dateFormat: "dd-mm-yy",
						maxDate:+0,
					 ...
					 onSelect: function(d,i){
						  if(d !== i.lastVal){
							var linkout=Linkkqxs+'ngay-'+d+'';
							  if (((linkout).split("#"))[0] == window.location.pathname)
								{
								KQXSOnload(linkout);
...
function setupadvclose()
{
$(".advgroup").each(function(index, element) {
    id=$(this).attr("data-id");
	if($(this).find(".adv").length>0)
	$(this).prepend('<div class="menuadvg"><button>Ẩn Quảng Cáo</button></div>');
	
	
});

if(getCookie("closeadv")==1)
	{
		$(".advgroup .menuadvg button").html('Hiện Quảng Cáo');
		$(".advgroup .menuadvg button").addClass("closed");
		$(".advgroup .adv").hide();
		$(".advgroup .ads_space").hide();
	}
	else
	{
		$(".advgroup .adv").show();
		$(".advgroup .ads_space").show();
	}

$(".advgroup button").click(function(e) {
        if($(this).hasClass("closed"))
		{
		$(".advgroup .menuadvg button").html('Ẩn Quảng Cáo');
		$(".advgroup .menuadvg button").removeClass("closed");
		setCookie("closeadv",0,0);
		$(".advgroup .adv").show();
		$(".advgroup .ads_space").show();
...
$(".onpagelink").each(function(index, element) {
    	setActive($(this));
});
...
var live_key=''; var wtime=1; var old_wtime=0;
function onofsound() {
	if (sound == true) {
		sound = false;
...
function xlKQXS(value) {
	...
	for (i = 0; i < j; i++) {
		value = value.replace('+', '<span class="runLoto loto_9">' + loto + '</span>');
		loto = loto + 3;
	...
	value = value.replace('***', '*');
	value = value.replace('**', '*');
	value = value.replace('*', '<span class="waitLoto"></span>');
...
function xlKHXS(kh) {
	...
	for (var i = 0; i < kh.length; i++) {
		if (kh[i] != "") {
			tam = kh[i].replace('*', '<span class="waitLoto"></span>');
			loto = Math.floor((Math.random() * 15) + 1);
			tam = tam.replace('+', '<span class="runLoto loto_15">' + loto + '</span>');
			str += '<span>' + tam + '</span>';
...
function setKQXSDaySo(id, val) {
	...
	if (waitingNumber != "") {
		if (!isNaN(val)) {
			...
			hdv = right(hsc, 1);
			if (hc == num && hdv == num) {
				$(id).html(left(val, val.length - 2) + '<span class="ketquadaysoHightlight clicked cahaihang">' + hsc + '</span>');
			}
			else if (hc == num) {
				$(id).html(left(val, val.length - 2) + '<span class="ketquadaysoHightlight clicked hangdonvi">' + hsc + '</span>');
			}
			else if (hdv == num) {
				$(id).html(left(val, val.length - 2) + '<span class="ketquadaysoHightlight clicked hangchuc">' + hsc + '</span>');
			}
		}
...
function setKQXSMax4D(id, val) {
	$(id).attr("data", val);
	$(id).html(xlKQXSMax4D(val));
	tick($(id));
	
}
function setSoLuongTrung(id, val) {
	if (!isNaN(val)) {
		$(id).html(addCommas(val));
	}
	else
	$(id).html(xlKQXS(val));
	tick($(id));
...
function runloto() {
	setInterval(function() {
		$(" .loto_9").each(function(index, element) {
			so = parseInt($(this).html());
			so++;
			if (so > 9) so = 0;
			$(this).html(so);
		});
		$(" .loto_15").each(function(index, element) {
			so15 = parseInt($(this).html());
			so15++;
			if (so15 > 15) so15 = 0;
			$(this).html(so15);
		});
$(" .loto_45").each(function(index, element) {
			so45 = right("0" + Math.floor((Math.random() * 45) + 1),2);
			$(this).html(so45);
		});
	}, 180);
}
...
function thongbaonoxstt() {
	...
	if (hour < start_gio || (hour == start_gio & minute < start_phut)) {
		$("#statusbar").html('<div class="ttxstt_waitting">Đang chờ Xổ Số ' + mien + ' lúc ' + start_gio + 'h' + (('0' + start_phut).substr(('0' + start_phut).length - 2, 2)) + '’, Chúc các bạn may mắn!</div>		');
		$('#mnxstt_' + mienurl).removeAttr('class');
		$('#mnxstt_' + mienurl).addClass('icon wait');
	}
	else if ((hour < end_gio || (hour == end_gio && minute < end_phut))) {
		$("#statusbar").html('		<div class="ttxstt"><a href="/ket-qua-xo-so-' + mienurl + '.html">Đang tường thuật trực tiếp kết quả xổ số! » Chúc các bạn may mắn!</a></div>		');
		$('#mnxstt_' + mienurl).removeAttr('class');
		$('#mnxstt_' + mienurl).addClass('icon running');
	}
	else {
		$("#statusbar").html('		<div class="ttxstt_finish"><a href="/ket-qua-xo-so-' + mienurl + '.html">Xổ số ' + mien + ' đã kết thúc » Xem thêm kết quả xổ số theo ngày!</a></div>		');
		$('#mnxstt_' + mienurl).removeAttr('class');
...
function thongbaoxstt() {
	...
	$("#statusbar").html('	<div class="ttxstt">Đang tường thuật trực tiếp kết quả xổ số! » Chúc các bạn may mắn !</div>	');
	$('#mnxstt_' + mienurl).removeAttr('class');
	$('#mnxstt_' + mienurl).addClass('icon running');
}
...
function xlKQXSdtMega(value) {
	loto = Math.floor((Math.random() * 10) + 1);
	lotos = Math.floor((Math.random() * 45) + 1);
	var j = value.length;

	for (i = 0; i < j; i++) {
		
		
		value = value.replace('++', '<span class="runLoto loto_45 ">' + lotos + '</span>');
		value = value.replace('+', '<span class="runLoto loto_9">' + loto + '</span>');
		value = value.replace('*', '<span class="waitLoto"></span>');
		loto = loto + 3;
		if (loto >= 10) loto = loto - 10;
...
function showkqxs_namtrung(dai,kq)
{
	...
	var countloto = 0;
	var kq_old= {"date":"","lv":"","8":"**","7":"***","6":["****","****","****"],"5":"****",	"4":["*****","*****","*****","*****","*****","*****","*****"],"3":["*****","*****"],"2":"*****","1":"*****","0":"******"};
	...
	var duoi9 = "";
						...
						for (v = 8; v >= 7; v--) {
							if (kq[v]) {
								if (kq[v] != kq_old[v]) {
									setKQXSDaySo("#giai" + (v) + "_" + dai, kq[v]);
									if (!isNaN(kq_old[v])) {
										alertedited($("#giai" + (v) + "_" + dai));
									}
								}
							}
							...
							arr_loto[v] = (kq[v]);
							arr_lotoduoi[countloto] = right(kq[v], 2);
							countloto++;
						}
						...
						for (v = 0; v < 3; v++) {
							if (kq[6][ v] != "") {
								if (kq[6][ v] != kq_old[6][ v ]) {
									setKQXSDaySo("#giai6_" + (v+1) + "_" + dai, kq[6][ v]);
									if (!isNaN(kq_old[6][ v ])) {
										alertedited($("#giai6_" + (v+1) + "_" + dai));
									}
								}
							}
							else emptyobj("#giai6_" + (v+1) + "_" + dai);
							arr_loto[6] += ',' + (kq[6][ v]);
							arr_lotoduoi[countloto] = right(kq[6][ v], 2)
							countloto++;
						}
						...
						if (kq[5]) {
							if (kq[5] != kq_old[5]) {
								setKQXSDaySo("#giai5_" + dai, kq[5]);
								if (!isNaN(kq_old[5 ])) {
									alertedited($("#giai5_" + dai));
								}
							}
						}
						else emptyobj("#giai5_" + dai);
						arr_loto[5] = (kq[5]);
						arr_lotoduoi[countloto] = right(kq[5], 2);
						countloto++;

						
						for (v = 0; v < 7; v++) {
							if (kq[4][v]) {
								if (kq[4][ v] != kq_old[4][v]) {
									setKQXSDaySo("#giai4_" + (v+1) + "_" + dai, kq[4][v]);
									if (!isNaN(kq_old[4][v])) {
										alertedited($("#giai4_" + (v+1) + "_" + dai));
									}
								}
							}
							...
							arr_loto[4] += ',' + (kq[4][v]);
							arr_lotoduoi[countloto] = right(kq[4][v], 2);
							countloto++;
						}
						...
						for (v = 0; v <2; v++) {
							if (kq[3][v]) {
								if (kq[3][v] != kq_old[3][v]) {
									setKQXSDaySo("#giai3_" + (v+1) + "_" + dai, (kq[3][v]));
									if (!isNaN(kq_old[3][v])) {
										alertedited($("#giai3_" + (v+1) + "_" + dai));
									}
								}
							}
							...
							arr_loto[3] += ',' + (kq[3][v]);
							arr_lotoduoi[countloto] = right(kq[3][v], 2);
							countloto++;
						}
						...
						if (kq[0]) {
							if (kq[0] != kq_old[0]) {
								setKQXSDaySo("#giaidb_" + dai, (kq[0]));
								if (!isNaN(kq_old[0])) {
									alertedited($("#giaidb_" + dai));
								}
							}
						}
						...
						arr_loto[0] = (kq[0]);
						arr_lotoduoi[countloto] = right(kq[0], 2);
						...
						for (var x = 0; x <= 9; x++) {
							for (var y = 8; y >= 0; y--) {
								...
								for (var z = 0; z < tam.length; z++) {
									if (tam[z] != '') {
										if (right(tam[z], 1) == ('' + x)) if ((y == 4) && (z == 1)) 
										duoi[x] += '<div class="dd_' + y + ' gach4con">' + right(tam[z], 4) + '</div>';
										else duoi[x] += '<div class="dd_' + y + '">' + right(tam[z], 4) + '</div>';
									}
								}
							}
						}
						...
						for (var j = 0; j < arr_lotoduoi.length; j++) {
							...
							if ((arr_lotoduoi[j]).substr(0, 1) == '0') {
								duoi0 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '1') {
								duoi1 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '2') {
								duoi2 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '3') {
								duoi3 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '4') {
								duoi4 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '5') {
								duoi5 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '6') {
								duoi6 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '7') {
								duoi7 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '8') {
								duoi8 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
							if (arr_lotoduoi[j].substr(0, 1) == '9') {
								duoi9 += '<span class="dd_' + css + '">' + arr_lotoduoi[j] + "</span> ";
							}
						}
						$("#duoiloto" + dai + "_0").html(
						duoi0);
						$("#duoiloto" + dai + "_1").html(
						duoi1);
						$("#duoiloto" + dai + "_2").html(
						duoi2);
						$("#duoiloto" + dai + "_3").html(
						duoi3);
						$("#duoiloto" + dai + "_4").html(
						duoi4);
						$("#duoiloto" + dai + "_5").html(
						duoi5);
						$("#duoiloto" + dai + "_6").html(
						duoi6);
						$("#duoiloto" + dai + "_7").html(
						duoi7);
						$("#duoiloto" + dai + "_8").html(
						duoi8);
						$("#duoiloto" + dai + "_9").html(
						duoi9);
...
function showkqxs_bac(dai,kq)
{
	...
	var arr_loto = new Array();
	var countloto = 0;
	var kq_old={"date":"","1":"*****","2":["*****","*****"],"3":["*****","*****","*****","*****","*****","*****"],"4":["****","****","****","****"],"5":["****","****","****","****","****","****"],"6":["***","***","***"],"7":["**","**","**","**"],"0":"*****","kh":["*","*","*"]};
	var arr_lotoduoi = new Array();
	var duoi = new Array();
	var duoi0 = "";
	var duoi1 = "";
	...
	var duoi9 = "";
						...
						for (v = 0; v <2; v++) {
							if (kq[2][v]) {
								if (kq[2][v] != kq_old[2][v]) {
									setKQXSDaySo("#giai2_" + (v+1) + "_" + dai, (kq[2][v]));
									if (!isNaN(kq_old[2][v])) {
										alertedited($("#giai2_" + (v+1) + "_" + dai));
									}
								}
							}
							...
							arr_loto[2] += ',' + (kq[2][v]);
							arr_lotoduoi[countloto] = right(kq[2][v], 2);
							countloto++;
						}
						...
						for (v = 0; v < 6; v++) {
							if (kq[5][ v] != "") {
								if (kq[5][ v] != kq_old[5][ v ]) {
									setKQXSDaySo("#giai5_" + (v+1) + "_" + dai, kq[5][ v]);
									if (!isNaN(kq_old[5][ v ])) {
										alertedited($("#giai5_" + (v+1) + "_" + dai));
									}
								}
							}
							else emptyobj("#giai5_" + (v+1) + "_" + dai);
							arr_loto[5] += ',' + (kq[5][ v]);
							arr_lotoduoi[countloto] = right(kq[5][ v], 2)
							countloto++;
						}
						...
						for (v = 0; v < 4; v++) {
							if (kq[7][ v] != "") {
								if (kq[7][ v] != kq_old[7][ v ]) {
									setKQXSDaySo("#giai7_" + (v+1) + "_" + dai, kq[7][ v]);
									if (!isNaN(kq_old[7][ v ])) {
										alertedited($("#giai7_" + (v+1) + "_" + dai));
									}
								}
							}
							else emptyobj("#giai6_" + (v+1) + "_" + dai);
							arr_loto[7] += ',' + (kq[7][ v]);
							arr_lotoduoi[countloto] = right(kq[7][ v], 2)
							countloto++;
						}
						...
						for (var x = 0; x <= 9; x++) {
							for (var y =0; y <= 7; y++) {
								...
								for (var z = 0; z < tam.length; z++) {
									if (tam[z] != '') {
										if (right(tam[z], 1) == ('' + x)) duoi[x] += '<div class="dd_' + y + '">' + right(tam[z], 4) + '</div>';
									}
								}
							}
...
function showkqxs_dientoan(dai,kq)
{
	
		
	if(dai==113)
	{				
	var kq_old={"date":"","1":"*","2":"**","3":"***"};
	if(kqxs_old[dai])
		kq_old=kqxs_old[dai];
						...
						if (kq[2]) {							
								if (kq[2] != kq_old[2]) {
									$("#giai2_1_"+dai).html(xlKQXSdt(kq[2].substr(0, 1)));
									$("#giai2_2_"+dai).html(xlKQXSdt(kq[2].substr(1, 1)));
									newkqxs = 1;
								}
							}
							else 
							{
								emptyobj("#giai2_1_" + dai);
								emptyobj("#giai2_2_" + dai);
							}
						
						if (kq[3]) {							
								if (kq[3] != kq_old[3]) {
									$("#giai3_1_"+dai).html(xlKQXSdt(kq[3].substr(0, 1)));
									$("#giai3_2_"+dai).html(xlKQXSdt(kq[3].substr(1, 1)));
									$("#giai3_3_"+dai).html(xlKQXSdt(kq[3].substr(2, 1)));
									newkqxs = 1;
								}
							}
							else 
							{
								emptyobj("#giai3_1_" + dai);
								emptyobj("#giai3_2_" + dai);
								emptyobj("#giai3_3_" + dai);
							}
	...
	if(kqxs_old[dai])
		kq_old=kqxs_old[dai];
						...
						for(v=1;v<=6;v++)
						{
							if (kq[v]) {
								if (kq[v] != kq_old[v]) {
								$("#giai"+v+"_"+dai).html(xlKQXSdt(kq[v]));
								newkqxs = 1;
								}
							}
							else {
								emptyobj("#giai"+v+"_"+dai);
							}
						}
	...
	if(kqxs_old[dai])
		kq_old=kqxs_old[dai];
						...
						if (kq[1]) {
							if (kq[1] != kq_old[1]) {
								$("#giai1_1_"+dai).html(xlKQXSdt(kq[1].substr(0, 1)));
								$("#giai1_2_"+dai).html(xlKQXSdt(kq[1].substr(1, 1)));
								$("#giai1_3_"+dai).html(xlKQXSdt(kq[1].substr(2, 1)));
								$("#giai1_4_"+dai).html(xlKQXSdt(kq[1].substr(3, 1)));
								newkqxs = 1;
							}
						}
						else {
							emptyobj("#giai1_1_" + dai);
							emptyobj("#giai1_2_" + dai);
							emptyobj("#giai1_3_" + dai);
							emptyobj("#giai1_4_" + dai);
						}
	}
	...
	if(kqxs_old[dai])
		kq_old=kqxs_old[dai];
						...
						arrslt=["1","2","3"];
						for(kk=0;kk<arrslt.length;kk++)
						{
							v=arrslt[kk];	
							if (kq["sl_"+v]) {
								if (kq["sl_"+v] != kq_old["sl_"+v]) {								
								setSoLuongTrung($("#sl_giai"+v+"_"+dai),kq["sl_"+v]);
								newkqxs = 1;
								}
							}
							else {
								emptyobj("#sl_giai"+v+"_"+dai);
							}
						}
						
						setSoLuongTrung($("#sl_jackpot_"+dai),kq["sl_jackpot"]);
						
						
						if (kq["jackpot"]) {
							if (kq["sl_jackpot"] != kq_old["sl_jackpot" ] || kq["jackpot"]!=kq_old["jackpot"]) {
								
								if(!isNaN(kq["jackpot"]))
								$("#jackpot_"+dai).html(addCommas( kq["jackpot"]));
								else
								$("#jackpot_"+dai).html(xlKQXSdt(kq["jackpot"]));

								newkqxs = 1;
								
								if (isNaN(kq["jackpot"]))
								{
									 $(".jackpot_next_val").html(xlKQXSdt(kq["jackpot"]));
									 $("#jackpot_n_"+dai).html(xlKQXSdt("*"));
								}
								else 
								{
									if(!isNaN(kq["sl_jackpot"]) && parseInt(kq["sl_jackpot"])>0)
									{
									$(".jackpot_next_val").html("12,000,000,000");									
									$("#jackpot_n_"+dai).html(addCommas( parseInt(kq["jackpot"]) / parseInt(kq["sl_jackpot"] ) ));									
									}
									else	
									{								
									$(".jackpot_next_val").html(addCommas(kq["jackpot"]));
									$("#jackpot_n_"+dai).html(addCommas( kq["jackpot"]));
									}
								}
							}
						}
	...
	if(dai==117)
	{				
	var kq_old={"date":"","1":"*","2":"*","3":"*","4":"*","5":"*","6":"*","7":"*","sl_jackpot":"*","sl_jackpot2":"*","sl_1":"*","sl_2":"*","sl_3":"*","jackpot":"***********","jackpot2":"***********"};
	if(kqxs_old[dai])
		kq_old=kqxs_old[dai];
						...
						setSoLuongTrung($("#sl_jackpot_"+dai),kq["sl_jackpot"]);
						setSoLuongTrung($("#sl_jackpot2_"+dai),kq["sl_jackpot2"]);
						...
						if (kq["sl_jackpot"]) {
							if (kq["sl_jackpot"] != kq_old["sl_jackpot" ] || kq["jackpot"]!=kq_old["jackpot"]) {
								...
								else 
								{
									if(!isNaN(kq["sl_jackpot"]) && parseInt(kq["sl_jackpot"])>0)
									{
									$(".jackpot_next_val").html("30,000,000,000");									
									$("#jackpot_n_"+dai).html(addCommas( parseInt(kq["jackpot"]) / parseInt(kq["sl_jackpot"] ) ));									
									}
									else	
									{								
							...
							emptyobj("#jackpot_n_"+dai);
						}
						
						
						if (kq["sl_jackpot2"]) {
							if (kq["sl_jackpot2"] != kq_old["sl_jackpot2" ] || kq["jackpot2"]!=kq_old["jackpot2"]) {
								
								if(!isNaN(kq["jackpot2"]))
								$("#jackpot2_"+dai).html(addCommas( kq["jackpot2"]));
								else
								...
								if (isNaN(kq["jackpot2"]))
								{
									 $(".jackpot2_next_val").html(xlKQXSdt(kq["jackpot2"]));
								}
								else 
								{
									if(!isNaN(kq["sl_jackpot2"]) && parseInt(kq["sl_jackpot2"])>0)
									{
									$(".jackpot2_next_val").html("3,000,000,000");									
									$("#jackpot2_n_"+dai).html(addCommas( parseInt(kq["jackpot2"]) / parseInt(kq["sl_jackpot2"] ) ));									
									}
									else	
									{								
									$(".jackpot2_next_val").html("+ " + addCommas(kq["jackpot2"]));
	...
	if(dai==116)
	{				
	var kq_old={"date":"","a":"****","b":"****","c":"****","d":"****","e":"****","g":"****","1":"****","2":["****","****"],"3":["****","****","****"],"4":"***","5":"**","sl_1":"*","sl_2":"*","sl_3":"*","sl_4":"*","sl_5":"*"};
	if(kqxs_old[dai])
		kq_old=kqxs_old[dai];
						
						arrkq=["a","b","c","d","e","g","1","4","5"];
						
						for(kk=0;kk<arrkq.length;kk++)
						{
							v=arrkq[kk]+"";console.log(kq[v]);
							try{
							if (kq[v]) {
								if (kq[v] != kq_old[v]) {
								setKQXSMax4D($("#giai"+v+"_"+dai),kq[v]);
								newkqxs = 1;
								}
							}
							else {
						...
						for(v=0;v<2;v++)
						{
							if (kq[2][v]) {
								if (kq[2][v] != kq_old[2][v]) {
								setKQXSMax4D($("#giai2_"+(v+1)+"_"+dai),kq[2][v]);
								newkqxs = 1;
								}
							}
							else {
								emptyobj("#giai2_"+(v+1)+"_"+dai);
							}
						}
						...
						for(v=0;v<3;v++)
						{
							if (kq[3][v]) {
								if (kq[3][v] != kq_old[3][v]) {
								setKQXSMax4D($("#giai3_"+(v+1)+"_"+dai),kq[3][v]);
								newkqxs = 1;
								}
							}
							else {
								emptyobj("#giai3_"+(v+1)+"_"+dai);
							}
						}
...
var oldreturntime = "";
var countime = 0;
function loadXSTT_Dai(strUrlaj)
{
	...
	$.ajax({
		...
		success: function() {
			if(kqxs[dai])
			{
				if(kqxs[dai]['date']==curentdate)
				{	
					
					
					
						if(mienid==2)
						showkqxs_bac(dai,kqxs[dai]);
						else
							if(mienid==1 || mienid==3)
						showkqxs_namtrung(dai,kqxs[dai]);
					else
						showkqxs_dientoan(dai,kqxs[dai]);
				...
				curentdate=kqxs[dai]['date'];
				kqxs = new Array();
					  kqxs_old = new Array();
				setTimeout(function() {
				getajaxxstt('box_xstt', '/xstt/tplDai.php?id=' + daiID + '', "_=" + (new Date()).getTime());
...
function loadXSTT_Mien(strUrlaj) {
	...
	if (returntime >0) {
		if (returntime != oldreturntime) {
			ajaxcache = true;
			str_province = 'k='+live_key+'&_=' +returntime;
			countime = 0;
		}
		else {
			countime++;
			ajaxcache = true;
			str_province = 'k='+live_key+'&_=' + returntime+ '_' + countime;
		}
		oldreturntime = returntime;
	}
	...
	$.ajax({
		...
		success: function() {
			...
			if (xoso[mienid].cf.time == lasttime ) {	
				...
				if (runxstt == "0") {
					...
					if(mienid==2)
					{
					for (var i = 0; i < ( xoso[mienid].cf.dt).length; i++)
					{
						dai=xoso[mienid].cf.dt[i];
						kq=xoso[mienid].kq[dai];
						kqxs[dai]=kq;
						showkqxs_dientoan(dai,kq);
					}
					}
				...
				var kkk = setTimeout(function() {
					vsit = 0;
					lasttime =  xoso[mienid].cf.time;
					getajaxxstt('box_xstt', '/xstt/tplMien.php?id=' + mienid + '', "cache=" + (new Date()).getTime());
					 kqxs = new Array();
					  kqxs_old = new Array();
				}, radom * 1000);
			}
...
function loadXSTT_3Mien(strUrlaj) {
	sound=true;
	var kq= new Object();
	newkqxs = 0;
	var dai=0;
	...
	$.ajax({
		...
		success: function() {
			runxstt=0;
			for(mienid=1;mienid<=3;mienid++)
			if( xoso[mienid].cf.run==1) { 
			
			runxstt=1; delay = xoso[mienid].cf.delay;
...
function loadKetquaMega645(strUrlaj, str_province) {
	...
	if (returntime >0) {
		if (returntime != oldreturntime) {
			ajaxcache = true;
			str_province = str_province + '&t=' +returntime;
			countime = 0;
		}
		else {
			countime++;
			ajaxcache = true;
			str_province = str_province + '&t=' + returntime+ '_' + countime;
		}
		oldreturntime = returntime;
	}
	...
	$.ajax({
		...
		cache: ajaxcache,
		success: function() {
			if (newtime == lasttime || changeDomain == true) {
				...
				if (runtt == 0) {
					...
					if (vsit == 1) thongbaoxstt();

					
					{
						for (v = 1; v <= 6; v++) {
							if (kqxs["Tdt6x45_G" + v]) {
								if (kqxs["Tdt6x45_G" + v] != kqxs["Tdt6x45_G" + v ]) {
									$("#DT6x45_G" + v).html(xlKQXSdtMega(kqxs["Tdt6x45_G" + v]));
									kqxs["Tdt6x45_G" + v ] = kqxs["Tdt6x45_G" + v];
									newkqxs = 1;
								}
							}
							else {
						...
						if (kqxs["Tdt6x45_Giatri"]) {
							if (kqxs["Tdt6x45_Giatri"] != kqxs["Tdt6x45_Giatri" ]) {
								if (kqxs["Tdt6x45_Giatri"] == "*") $(".jackpot_val").html(xlKQXSdt(kqxs["Tdt6x45_Giatri"]));
								else
								{ $(".jackpot_val").html(addCommas(kqxs["Tdt6x45_Giatri"])); 
						...
						if (kqxs["Tdt6x45_SLDB"]) {
							if (kqxs["Tdt6x45_SLDB"] != kqxs["Tdt6x45_SLDB" ] || kqxs["Tdt6x45_Giatri"]!=kqxs["Tdt6x45_Giatri_old"]) {
								$("#DT6X45_S_JACKPOT").html(xlKQXSdt(kqxs["Tdt6x45_SLDB"]));
								kqxs["Tdt6x45_SLDB" ] = kqxs["Tdt6x45_SLDB"];
								kqxs["Tdt6x45_Giatri_old"]=kqxs["Tdt6x45_Giatri"];
								newkqxs = 1;
								
								if (kqxs["Tdt6x45_Giatri"] == "*") $(".jackpot_next_val").html(xlKQXSdt(kqxs["Tdt6x45_Giatri"]));
								else 
								{
									if(!isNaN(kqxs["Tdt6x45_SLDB"]) && parseInt(kqxs["Tdt6x45_SLDB"])>0)
									{
									$(".jackpot_next_val").html("12,000,000,000");
									
									$("#DT6X45_S_JACKPOT_n").html(addCommas( parseInt(kqxs["Tdt6x45_Giatri"]) / parseInt(kqxs["Tdt6x45_SLDB"] ) ));
									
									}
									else	
									{								
									$(".jackpot_next_val").html("+ " + addCommas(kqxs["Tdt6x45_Giatri"]));
									$("#DT6X45_S_JACKPOT_n").html(addCommas(kqxs["Tdt6x45_Giatri"]));
									}
								}
							}
						}
						else {
							emptyobj("#DT6X45_S_JACKPOT");
						}
						
						
						if (kqxs["Tdt6x45_SL1"]) {
							if (kqxs["Tdt6x45_SL1"] != kqxs["Tdt6x45_SL1" ]) {
								$("#DT6X45_S_G1").html(xlKQXSdt(kqxs["Tdt6x45_SL1"]));
								kqxs["Tdt6x45_SL1" ] = kqxs["Tdt6x45_SL1"];
								newkqxs = 1;
							}
						}
						else {
							emptyobj("#DT6X45_S_G1");
						}
						if (kqxs["Tdt6x45_SL2"]) {
							if (kqxs["Tdt6x45_SL2"] != kqxs["Tdt6x45_SL2" ]) {
								$("#DT6X45_S_G2").html(xlKQXSdt(kqxs["Tdt6x45_SL2"]));
								kqxs["Tdt6x45_SL2" ] = kqxs["Tdt6x45_SL2"];
								newkqxs = 1;
							}
						}
						else {
							emptyobj("#DT6X45_S_G2");
						}
						if (kqxs["Tdt6x45_SL3"]) {
							if (kqxs["Tdt6x45_SL3"] != kqxs["Tdt6x45_SL3" ]) {
								$("#DT6X45_S_G3").html(xlKQXSdt(kqxs["Tdt6x45_SL3"]));
								kqxs["Tdt6x45_SL3" ] = kqxs["Tdt6x45_SL3"];
								newkqxs = 1;
							}
						}
						else {
							emptyobj("#DT6X45_S_G3");
						}
				...
				var kkk = setTimeout(function() {
					...
					lasttime = newtime;
					kqxs = new Array();
					getajaxxstt('box_xstt', '/xstt/m' + style + '.php', "cache=" + (new Date()).getTime());
				}, radom * 1000);
			}
		}
...
function loadKetquaMax4D(strUrlaj, str_province) {
		...
		success: function() {
			if (newtime == lasttime || changeDomain == true) {
				...
				if (runtt == 0) {
					...
					if (kqxs[tinhcode + '_ListKQ'].length > 5) {
						
						$(".xsm4d .returnkqxs").show();
						if (kqxs[tinhcode + '_ListKQ'] != kqxs[tinhcode + '_ListKQ' ])
						{

						var listtrung = kqxs[tinhcode + '_ListKQ'].split("-");
						setKQXSMax4D("#itemkqA .kq", listtrung[0]);
						setKQXSMax4D("#itemkqB .kq", listtrung[1]);
						setKQXSMax4D("#itemkqC .kq", listtrung[2]);
						setKQXSMax4D("#itemkqD .kq", listtrung[3]);
						setKQXSMax4D("#itemkqE .kq", listtrung[4]);
						setKQXSMax4D("#itemkqG .kq", listtrung[5]);
						kqxs[tinhcode + '_ListKQ' ]=kqxs[tinhcode + '_ListKQ'];
						newkqxs = 1;
						}
					}
					else
					{
					$(".xsm4d .returnkqxs").hide();	
					}

					
					v = 1; {
						if (kqxs[tinhcode + '_G' + v]) {
							if (kqxs[tinhcode + '_G' + v] != kqxs[tinhcode + '_G' + v ]) {
								setKQXSMax4D(".max4d_g1 .lq_1", kqxs[tinhcode + '_G' + v]);
								if (!isNaN(kqxs[tinhcode + '_G' + v ])) {
									alertedited($(".max4d_g1 .lq_1"));
								}
								kqxs[tinhcode + '_G' + v ] = kqxs[tinhcode + '_G' + v];
								newkqxs = 1;
							}
						}
						else emptyobj(".max4d_g1 .lq_1");
					}

					
					for (v = 1; v <= 2; v++) {
						if (kqxs[tinhcode + '_G2_' + v]) {
							if (kqxs[tinhcode + '_G2_' + v] != kqxs[tinhcode + '_G2_' + v ]) {
								setKQXSMax4D(".max4d_g2 .lq_" + v, kqxs[tinhcode + '_G2_' + v]);
								if (!isNaN(kqxs[tinhcode + '_G2_' + v ])) {
									alertedited($(".max4d_g2 .lq_" + v));
								}
								kqxs[tinhcode + '_G2_' + v ] = kqxs[tinhcode + '_G2_' + v];
								newkqxs = 1;
							}
						}
						else emptyobj(".max4d_g2 .lq_" + v);
					}
					
					for (v = 1; v <= 3; v++) {
						if (kqxs[tinhcode + '_G3_' + v]) {
							if (kqxs[tinhcode + '_G3_' + v] != kqxs[tinhcode + '_G3_' + v ]) {
								setKQXSMax4D(".max4d_g3 .lq_" + v, kqxs[tinhcode + '_G3_' + v]);
								if (!isNaN(kqxs[tinhcode + '_G3_' + v ])) {
									alertedited($(".max4d_g3 .lq_" + v));
								}
								kqxs[tinhcode + '_G3_' + v ] = kqxs[tinhcode + '_G3_' + v];
								newkqxs = 1;
							}
						}
						else emptyobj(".max4d_g3 .lq_" + v);
					}

					
					for (v = 1; v <= 2; v++) {
						if (kqxs[tinhcode + '_GKK' + v]) {
							if (kqxs[tinhcode + '_GKK' + v] != kqxs[tinhcode + '_GKK' + v ]) {
								setKQXSMax4D(".max4d_gkk" + v + " .lq_1", kqxs[tinhcode + '_GKK' + v]);
								if (!isNaN(kqxs[tinhcode + '_GKK' + v ])) {
									alertedited($(".max4d_gkk" + v + " .lq_1"));
								}
								kqxs[tinhcode + '_GKK' + v ] = kqxs[tinhcode + '_GKK' + v];
								newkqxs = 1;
							}
						}
						else emptyobj(".max4d_gkk" + v + " .lq_1");
					}
					
					
					for (v = 1; v <=3; v++) {
						if (kqxs['max4d_SL' + v]) {
							if (kqxs['max4d_SL' + v] != kqxs['max4d_SL' + v ]) {
								setKQXSMax4D("#slGiai" + v , kqxs['max4d_SL' + v]);
								if (!isNaN(kqxs['max4d_SL' + v ])) {
									alertedited($("#slGiai" + v));
								}
								kqxs['max4d_SL' + v ] = kqxs['max4d_SL' + v];
								newkqxs = 1;
							}
						}
						else emptyobj("#slGiai" + v );
					}
					
					for (v = 1; v <=2; v++) {
						if (kqxs['max4d_SLkk' + v]) {
							if (kqxs['max4d_SLkk' + v] != kqxs['max4d_SLkk' + v ]) {
								setKQXSMax4D("#slGiaikk" + v , kqxs['max4d_SLkk' + v]);
								if (!isNaN(kqxs['max4d_SLkk' + v ])) {
									alertedited($("#slGiaikk" + v));
								}
								kqxs['max4d_SLkk' + v ] = kqxs['max4d_SLkk' + v];
								newkqxs = 1;
							}
						}
						else emptyobj("#slGiaikk" + v );
					}
...
{
$(document).ready(function(e) {
	var minwindowMaxchat=1280;
var windowsiteMaxchat=1000;
var stylechat="mini";
if(window.innerWidth >=minwindowMaxchat)
stylechat="right";

$("body").append('<div id="chatbox" class="chatboxs chatbox_'+stylechat+'"><span class="opentChatbox"></span><span class="closeChatbox">X</span><div class="chatboxFrame"></div></div>');
$(".opentChatbox").click(function(e) {
	$(".opentChatbox").addClass("clicked").hide();
	$(".closeChatbox").show();
	if(stylechat=="right")
	{
	var rightchatwith= $("body").width() -  windowsiteMaxchat;
	if(rightchatwith >500) rightchatwith=500;
	
	$("body >div:first").animate({marginRight:rightchatwith},300);
    $(".chatboxFrame").html('<div style="border-left:1px solid #ccc;" ><iframe id="ifchat" frameborder="0"  src="/'+'/chat.trumxoso.com/?f=r" width="100%" height="100%"></iframe></div>');
	$("#ifchat").height(window.innerHeight );
	$("#ifchat").animate({width: rightchatwith - 2 },300);
	}
	else
	{
    $(".chatboxFrame").html('<div ><iframe id="ifchat" frameborder="0"  src="/'+'/chat.trumxoso.com/" width="100%" height="100%"></iframe></div>');
	$("#ifchat").height(window.innerHeight *2/3);
	$("#ifchat").animate({ width: 300},300);
	}
		setCookie("onchat",1,7);

});
$(".closeChatbox").click(function(e) {
setCookie("onchat",0,7);
	$(this).hide();$(".opentChatbox").show();
    $(".chatboxFrame").html("");
	$("body >div:first").animate({marginRight:0},300);
});

 if(getCookie("onchat")=="1" )
   {
	$(".opentChatbox").click();
   }
});	
...
setTimeout(function(){
	var strVar="";
strVar += "<div id='fbox-background'>";
strVar += "  <div id='fbox-close'> <\/div>";
strVar += "  <div id='fbox-display'>";
strVar += "    <div class=\"likeheader\">Quyền trợ giúp Nhờ Người Thân !<\/div>";
strVar += "    <div id='fbox-button'> <\/div>";
strVar += "    <div style=\"text-align: center; padding:10px; \">";
strVar += "      <p style=\"text-align: center;\"> <span style=\"color:#f00;\"><strong>Chia Sẽ Ngay &amp; Lu&ocirc;n Nh&eacute; !<\/strong><\/span><\/p>";
strVar += "      <p style=\"text-align: center;\"> <b>H&atilde;y hỗ trợ ch&uacute;ng t&ocirc;i ph&aacute;t triển th&ecirc;m nhiều chức năng hay hơn nữa bằng việc chia sẽ&nbsp;đến bạn b&egrave; của bạn website của ch&uacute;ng t&ocirc;i <\/b><\/p>";
strVar += "      <br>";
strVar += "      <div class=\"td-post-sharing td-post-sharing-bottom td-with-like\"><span class=\"td-post-share-title\">Chia sẻ:<\/span>";
strVar += "        <div class=\"td-default-sharing\"> <a class=\"td-social-sharing-buttons td-social-facebook\" href=\"http:\/\/www.facebook.com\/sharer.php?u=www.xosohomnay.com.vn\" onClick=\"window.open(this.href, 'mywin','left=50,top=50,width=600,height=350,toolbar=0'); return false;\"><i class=\"td-icon-facebook\"><\/i>";
strVar += "          <div class=\"td-social-but-text\">Facebook<\/div>";
strVar += "          <\/a> <a class=\"td-social-sharing-buttons td-social-twitter\" href=\"https:\/\/twitter.com\/intent\/tweet?text=&url=www.xosohomnay.com.vn&via=XSMN.net\" target=\"_blank\" onClick=\"window.open(this.href, 'mywin','left=50,top=50,width=600,height=350,toolbar=0'); return false;\" ><i class=\"td-icon-twitter\"><\/i>";
strVar += "          <div class=\"td-social-but-text\">Twitter<\/div>";
strVar += "          <\/a> <a class=\"td-social-sharing-buttons td-social-google\" href=\"http:\/\/plus.google.com\/share?url=www.xosohomnay.com.vn\" onClick=\"window.open(this.href, 'mywin','left=50,top=50,width=600,height=350,toolbar=0'); return false;\"><i class=\"td-icon-googleplus\"><\/i>Google +<\/a><\/div>";
strVar += "      <\/div>";
strVar += "    <\/div>";
strVar += "    <p style=\"text-align: center;\"> <span style=\"color:#f00;\"><strong>Cảm ơn qu&yacute; kh&aacute;ch đ&atilde; sử dụng website của ch&uacute;ng t&ocirc;i !<\/strong><\/span><\/p>";
strVar += "    <p> <a href=\"https:\/\/goo.gl\/zsrinv\" target=\"_blank\"><img alt=\"\" border=\"0\" height=\"58\" src=\"\/upload\/images\/banner\/download_app_xosohomnay_320x70.gif\" width=\"300\" \/><\/a><\/p>";
strVar += "  <\/div>";
strVar += "<\/div>";
strVar += "<\/div>";
strVar += "";
    ...
    function setDateMax4D(is)
  {
	  ...
	  $(document).ready(function(e) {
		  $("#mientol").change(function(e) {
            if($("#mientol option:selected").attr("data-id")==115)
			{
				setDateMega645(true);
			}
			else
        ...
        $(".btnprint input").click(function(e) {
            $("#frminkqtol #pagetol").val($(this).attr("data-id"));
			 var linkout="/in-ve-do/"+$("#mientol").val()+"#page="+ $(this).attr("data-id")+ "&date="+ $("#vdntoll").val();
			window.open(linkout);
			return false
        });
    });